<template>
    <div class="max-w-6xl mx-auto p-4">
      <div class="flex justify-between items-center mb-8">
        ...
        <div class="flex gap-3">
          <select v-model="sort" class="px-4 py-2 border rounded-lg bg-white shadow-sm hover:shadow transition-all">
            <option value="copied_count">복사 횟수순</option>
            <option value="created_at">최신순</option>
          </select>
          <select v-model="size" class="px-4 py-2 border rounded-lg bg-white shadow-sm hover:shadow transition-all">
        ...
        >
          <div class="p-5 space-y-4">
            <div class="flex justify-between items-start">
              <h3 class="font-semibold text-xl hover:text-blue-600 transition-colors">{{ trip.content }}</h3>
              <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-1.5 rounded-full">
              ...
              >
                <div class="relative overflow-hidden rounded-lg">
                  ...
                  <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/50 to-transparent p-3">
                    <p class="text-white font-medium">{{ attraction.sequence }}. {{ attraction.title }}</p>
                    <p class="text-white/80 text-sm truncate">{{ attraction.address }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="pt-3 flex gap-2">
              <button 
                ...
                class="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 
      ...
      <div class="mt-8 flex justify-center gap-2">
        <button 
          ...
          :key="pageNum"
          @click="page = pageNum"
          :class="['px-4 py-2 rounded-lg font-medium transition-all duration-200 hover:-translate-y-0.5', 
                  page === pageNum ? 'bg-blue-500 text-white shadow-md' : 'bg-white hover:bg-gray-50']"
        >
          {{ pageNum }}
        </button>
      </div>
  ...
  <div v-if="selectedTrip" 
    ...
    <div class="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" @click.stop>
      <div class="sticky top-0 bg-white border-b p-4 flex justify-between items-center z-10">
        <div>
          <h2 class="text-2xl font-bold">{{ selectedTrip.content }}</h2>
          <p class="text-gray-600">{{ selectedTrip.startDate }} ~ {{ selectedTrip.endDate }}</p>
        </div>
      ...
      <div class="p-6">
        <div v-for="day in getDays(selectedTrip)" :key="day" class="mb-8 last:mb-0">
          <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center gap-2">
              <h3 class="text-xl font-bold">Day {{ day }}</h3>
              <span class="text-sm text-gray-500">{{ selectedTrip.attractions.filter(a => a.day === day).length }}개의 장소</span>
            </div>
            <div class="h-px bg-gray-200 flex-1"></div>
          </div>

          <div class="grid gap-4">
            <div v-for="attraction in getDayAttractions(selectedTrip, day)" 
                 :key="attraction.no"
